...
public class Player2 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        isShooting = PlayerPrefs.GetInt("isShooting") == 1 ? true : false;
        ...
        if (Input.GetKeyDown(KeyCode.Space) || isShooting/*|| Input.GetMouseButtonDown(0)*/) {
            Shoot();
            isShooting = false;
            PlayerPrefs.SetInt("isShooting", isShooting ? 1 : 0);
    ...
    private void Shoot()
    {
        Bullet2 bullet = Instantiate(bulletPrefab, transform.position, transform.rotation);
